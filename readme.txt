...
* Send only the required Atlas refresh token during revalidation. The token already bypasses stored Atlas HTML, so redundant request headers can no longer trigger unrelated no-store responses; private response headers remain protected from cache storage.
...
* Fix false detection of custom WP_CACHE definitions when wp-config.php only mentions WP_CACHE in comments or strings.
* Accept enabled WP_CACHE definitions with trailing comments without modifying wp-config.php.

= 0.1.15 =

* Add configurable automatic site revalidation through WP-Cron (seven days by default).
* Allow post types to be excluded from cache storage and revalidation.
* Add browser-cache duration settings by file type and Apache gzip fallback controls.
* Expand diagnostics for HTTP compression and runtime cache state.
...
* Add cache warm-up priorities for taxonomy archive URLs.

= 0.1.11 =

* Release the current Atlas Cache build with default-off cache, automatic revalidation when enabling cache, post type warm-up priorities, sitemap fallback, media URL filtering and optional .htaccess browser cache rules.
